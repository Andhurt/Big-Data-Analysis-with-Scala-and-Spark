...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val columnCategories =  Vector(
      Vector("t01", "t03", "t11", "t1801", "t1803").map((1,_)),
      Vector("t05", "t1805").map((2,_)),
      Vector("t02", "t04", "t06", "t07", "t08", "t09","t10", "t12", "t13", "t14", "t15", "t16" , "t18").map((3,_))).flatten


    def getCategoryColumn(columnName: String): Int = {
      columnCategories.foldLeft(0)((acc, x) => if (columnName.startsWith(x._2) && acc == 0) x._1 else acc)
    }

     columnNames.foldLeft((List[Column](), List[Column](), List[Column]()))((acc, x) => {

      val categoryColumn = getCategoryColumn(x)

      if (categoryColumn == 1) (new Column(x)::acc._1, acc._2, acc._3)
        else if (categoryColumn== 2) (acc._1, new Column(x)::acc._2, acc._3)
        else if (categoryColumn== 3) (acc._1, acc._2, new Column(x)::acc._3)
        else (acc._1, acc._2, acc._3)
    })
  }
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map {
      x =>
        TimeUsageRow(x.getAs[String]("working"),
          x.getAs[String]("sex"),
          x.getAs[String]("age"),
